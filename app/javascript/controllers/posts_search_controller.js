...
export default class extends Controller {
  ...
  connect() {
    ...
    if (!this.appId || !this.apiKey || !this.postsIx) return;
    this.client = algoliasearch(this.appId, this.apiKey);
    this.element.setFilterCallback?.(() => true);
    this._seq = 0;
    this._loadingTimer = null;
    this._debounced = this._debounce((q) => this._searchPosts(q), this.debounceValue);
    this.inputTarget.addEventListener("input", (e) => this._debounced(e.target.value));
    if (this.prefetchValue) this._searchPosts("");
  }
  ...
  _renderPosts(hits) {
    ...
    hits.forEach((hit) => {
      ...
      titleEl.innerHTML = hl.title?.value ?? (hit.title || "Untitled");
      subEl.innerHTML = sn.content?.value ?? (hit.content || "");
